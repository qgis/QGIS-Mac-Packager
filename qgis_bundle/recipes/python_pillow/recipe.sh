...
function fix_binaries_python_pillow() {
  ...
  for i in \
    _imaging.cpython-${VERSION_major_python//./}-darwin.so \
    _imagingcms.cpython-${VERSION_major_python//./}-darwin.so \
    _imagingft.cpython-${VERSION_major_python//./}-darwin.so \
    _imagingmath.cpython-${VERSION_major_python//./}-darwin.so \
    _imagingmorph.cpython-${VERSION_major_python//./}-darwin.so \
    _imagingtk.cpython-${VERSION_major_python//./}-darwin.so \
    _webp.cpython-${VERSION_major_python//./}-darwin.so
  do
  ...
  for j in \
      ...
      $LINK_libtiff
  do
    install_name_change $DEPS_LIB_DIR/$j @rpath/$j $pillow_EGG/PIL/_imaging.cpython-${VERSION_major_python//./}-darwin.so
  done

  install_name_change $DEPS_LIB_DIR/$LINK_little_cms2 @rpath/$LINK_little_cms2 $pillow_EGG/PIL/_imagingcms.cpython-${VERSION_major_python//./}-darwin.so
  install_name_change $DEPS_LIB_DIR/$LINK_freetype @rpath/$LINK_freetype $pillow_EGG/PIL/_imagingft.cpython-${VERSION_major_python//./}-darwin.so
  install_name_change $DEPS_LIB_DIR/$LINK_libwebp @rpath/$LINK_libwebp $pillow_EGG/PIL/_webp.cpython-${VERSION_major_python//./}-darwin.so
}
