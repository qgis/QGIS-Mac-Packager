...
function fix_binaries_python_psycopg2() {
  for i in \
    ...
    $LINK_libpq
  do
    install_name_change $DEPS_LIB_DIR/$i @rpath/$i $BUNDLE_PYTHON_SITE_PACKAGES_DIR/psycopg2/_psycopg.cpython-${VERSION_major_python//./}-darwin.so
  done
}

function fix_binaries_python_psycopg2_check() {
  verify_binary $BUNDLE_PYTHON_SITE_PACKAGES_DIR/psycopg2/_psycopg.cpython-${VERSION_major_python//./}-darwin.so
}
