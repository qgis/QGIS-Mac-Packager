...
function fix_binaries_python_opencv() {
  ...
  for i in \
    ...
    $LINK_openjpeg
  do
    install_name_change $DEPS_LIB_DIR/$i @rpath/$i $OPENCV_EGG_DIR/cv2/cv2.cpython-${VERSION_major_python//./}-darwin.so
  done
}
...
function fix_binaries_python_opencv_check() {
  ...
  verify_binary $OPENCV_EGG_DIR/cv2/cv2.cpython-${VERSION_major_python//./}-darwin.so
}
